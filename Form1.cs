    ...
    public partial class Form1 : Form
    {
        ...
        private void HPFButton_Click(object sender, EventArgs e)
        {
            ...
            while (lft > 0)
            {
                ...
                //if there is such a large gap between process that there are no ready process left 
                if (heap.IsEmpty)
                {
                ...
                //Add processes that arrived during context switch
                for (int i = 0; i < parr.Length; ++i)
                {
                    if (parr[i] != null && parr[i].AT <= CurrentTime)
                    {
        ...
        private void FCFSButton_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < parr.Length; ++i)
            {
                ...
                //Execute Process
                //Add Context Switch
                double from = CurrentTime + S;
                double to = CurrentTime + S + parr[i].BT;
        ...
        private void RRButton_Click(object sender, EventArgs e)
        {
            ...
            while (lft > 0)
            {
                ...
                double from = CurrentTime + S;
                double to = CurrentTime + S + ExecutionTime;
                ProcessQueue.Add(new Tuple<int, Tuple<double, double>>(id, new Tuple<double, double>(from, to)));
        ...
        private void SRTNButton_Click(object sender, EventArgs e)
        {
            ...
            //This PQ represents the processes in ready state (BT,ID)
            ...
            for (int idx = 0; idx < parr.Length; ++idx)
            {
                ...
                // Add processes that arrived during the Context Switch
                while (idx < parr.Length && parr[idx].AT <= current_time)
                {
                    heap.Add(new Tuple<double, int>(parr[idx].BT, parr[idx].ID));
                    idx++;
                ...
                heap.Add(cur);
                current_time = parr[idx].AT;
                if (heap.Min() == cur) current_time -= S;
            }
            ...
            while (!heap.IsEmpty)
            {
                var ccur = heap.Min();
                heap.DeleteMin();
                Finish[ccur.Item2] = current_time + S + ccur.Item1;

                ProcessQueue.Add(new Tuple<int, Tuple<double, double>>(ccur.Item2, new Tuple<double, double>(current_time + S, Finish[ccur.Item2])));
